...
def merge_region_boundary_cells(data, stride_x, stride_y, print_log=False):
    """
    Merge cells that are split across region boundaries from processing in patches.
    This is done in-place within the data list.
    ...
    data : dict
        Individual cell data and associated values generated by the 'compute_cell_results' function.
    stride_x : int
        Stride between patches in x-direction.
    stride_y : int
        Stride between patches in y-direction
    print_log : bool
    ...
    # get all cells on patch boundaries
    bcells = []
    for i, cell in enumerate(data['cells']):
        if data['dataVersion'] == 4:
            ...
            c = cell
        if (c['bbox'][0][0] % stride_x == 0 or c['bbox'][0][1] % stride_y == 0 or
                c['bbox'][1][0] % stride_x == stride_x-1 or c['bbox'][1][1] % stride_y == stride_y-1):
            border = make_full_contour(c['boundary'])
            coords = []
            for b in border:
                mod_x = b[0] % stride_x
                mod_y = b[1] % stride_y
                if mod_x == 0 or mod_x == stride_x-1:
                    coords.append(b)
                elif mod_y == 0 or mod_y == stride_y-1:
                    coords.append(b)
            bcells.append({
                'cells': [c],
                'coords': coords,
    ...
    print_info('Num boundary cells:', len(bcells))

    # go through boundary cells and group where touching
    for i in range(len(bcells)):
        for j in range(i+1, len(bcells)):
            share_boundary = False

            for c1 in bcells[i]['cells']:
                for c2 in bcells[j]['cells']:
                    if (c1['bbox'][0][0] == c2['bbox'][1][0] + 1 or
                            c1['bbox'][0][1] == c2['bbox'][1][1] + 1 or
                            c1['bbox'][1][0] == c2['bbox'][0][0] - 1 or
                            c1['bbox'][1][1] == c2['bbox'][0][1] - 1):
                        share_boundary = True
                        break
                if share_boundary:
                    break
            ...
            if share_boundary:
                ...
                for c1 in bcells[i]['coords']:
                    for c2 in bcells[j]['coords']:
                        dx = abs(c1[0] - c2[0])
                        dy = abs(c1[1] - c2[1])
                        if dx <= 1 and dy <= 1:
                            touching = True
                ...
                if touching:
                    bcells[j]['cells'].extend(bcells[i]['cells'])
                    bcells[j]['coords'].extend(bcells[i]['coords'])
                    bcells[j]['orig_idx'].extend(bcells[i]['orig_idx'])
                    bcells[i] = None
                    break

    if print_log:
        count_empty, count_whole, count_multipart = 0, 0, 0
        for bcell in bcells:
            if bcell is None:
                count_empty += 1
            elif len(bcell['cells']) == 1:
                count_whole += 1
            else:
                count_multipart += 1
        print_info('Num empty boundary cells:', count_empty)
        print_info('Num whole boundary cells:', count_whole)
        print_info('Num multipart boundary cells:', count_multipart)

    # merge multipart cells
    for i in range(len(bcells)):
        if bcells[i] is None or len(bcells[i]['cells']) == 1:
            continue

        min_extent = [bcells[i]['cells'][0]['boundary'][0][0], bcells[i]['cells'][0]['boundary'][0][1]]
        max_extent = [bcells[i]['cells'][0]['boundary'][0][0], bcells[i]['cells'][0]['boundary'][0][1]]
        for c in bcells[i]['cells']:
            for b in c['boundary']:
                if b[0] < min_extent[0]:
                    min_extent[0] = b[0]
                elif b[0] > max_extent[0]:
                    max_extent[0] = b[0]
                if b[1] < min_extent[1]:
                    min_extent[1] = b[1]
                elif b[1] > max_extent[1]:
                    max_extent[1] = b[1]
        extent = [max_extent[0] - min_extent[0] + 1, max_extent[1] - min_extent[1] + 1]

        mask = np.full((extent[1], extent[0]), LABEL_BACKGROUND, dtype=np.uint8)
        for c in bcells[i]['cells']:
            border = make_full_contour(c['boundary'])
            for b in border:
                mask[b[1]-min_extent[1], b[0]-min_extent[0]] = LABEL_CELL

        # get first pixel from top left
        first = None
        for y in range(mask.shape[0]):
            for x in range(mask.shape[1]):
                if mask[y, x] != LABEL_BACKGROUND:
                    first = (x, y)
                    break
            if first is not None:
                break

        full_boundary = get_cell_boundary(mask, first[0], first[1])
        for k in range(len(full_boundary)):
            for j in range(len(full_boundary[k])):
                full_boundary[k][j] = (full_boundary[k][j][0] + min_extent[0], full_boundary[k][j][1] + min_extent[1])

        # calculate merged cell info
        size = 0
        posneg = 0
        marker = None
        od = None
        ...
        for c in bcells[i]['cells']:
            size += c['size']
            posneg += c['size'] if c['positive'] else -c['size']
            center_x += (c['centroid'][0] * c['size'])
            center_y += (c['centroid'][1] * c['size'])
            if 'od' in c:
                if od is None:
                    od = 0
                od += c['od'] * c['size']
        ...
        if od is not None:
            od = round(od / size)
        center_x = round(center_x / size)
        center_y = round(center_y / size)

        merged_cell = {
            'size': size,
            'positive': posneg >= 0,
            'bbox': full_boundary[0],
            'centroid': (center_x, center_y),
            'boundary': make_simple_contour(full_boundary[1]),
        }
        if od is not None:
            merged_cell['od'] = od
        if marker is not None:
            merged_cell['marker'] = marker

        bcells[i]['merged'] = merged_cell

    if print_log:
        count_merged = 0
        count_parts = 0
        for bcell in bcells:
            if bcell is not None and 'merged' in bcell:
                count_merged += 1
                count_parts += len(bcell['cells'])
        print_info('Num merged cells:', count_merged)
        print_info('Num separate parts:', count_parts)
        print_info('Num removed cells:', count_parts-count_merged)

    # insert merged cells back into original data list
    print_info('Number of cells before merging:', len(data['cells']))
    count_merged, count_emptied = 0, 0
    for bcell in bcells:
        if bcell is None or 'merged' not in bcell:
            continue
        cell = bcell['merged']
        if data['dataVersion'] == 4:
            cell = encode_cell_data_v4(cell)
        elif data['dataVersion'] == 6:
            cell = encode_cell_data_v4(cell, v6=True)
        data['cells'][bcell['orig_idx'][0]] = cell
        count_merged += 1
        for i in range(1, len(bcell['orig_idx'])):
            data['cells'][bcell['orig_idx'][i]] = None
            count_emptied += 1

    print_info('Num merged cells:', count_merged)
    print_info('Num emptied cells:', count_emptied)
    if print_log:
        count_none = 0
        for c in data['cells']:
            if c is None:
                count_none += 1
        print_info('Num entries to remove:', count_none)

    # remove empty cells
    j = 0
    for i in range(len(data['cells'])):
        if data['cells'][i] is None:
            continue
        data['cells'][j] = data['cells'][i]
        j += 1
    for i in range(len(data['cells']) - j):
        data['cells'].pop()
    print_info('Number of cells after merging:', len(data['cells']))
