...
def get_mod_id_seg(dir_model):
    ...
    fns = [fn for fn in os.listdir(dir_model) if fn.endswith('.pth') and fn.startswith('latest_net_G')]
    model_names = [fn[:-4].replace('latest_net_G','') for fn in fns]
...
def get_input_id(dir_model):
    # assume we already know there are seg models - this check is intended to be done prior to calling this function
    fns = [fn for fn in os.listdir(dir_model) if fn.endswith('.pth') and fn.startswith('latest_net_G')]
    model_names_seg = [fn[:-4].replace('latest_net_G','')[1:] for fn in fns]
