...
class DeepLIIFKDModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if not hasattr(self,'mod_id_seg') and hasattr(opt,'mod_id_seg'): # use mod id seg from train opt file if available
            ...
            self.mod_id_seg = 'S'
        print('Initializing DeepLIIFKD model with segmentation modality id:',self.mod_id_seg)
        ...
        for i in range(1, self.opt.modalities_no + 1):
            self.loss_names.extend([f'G_GAN_{i}', f'G_L1_{i}', f'D_real_{i}', f'D_fake_{i}', f'G_KLDiv_{i}', f'G_KLDiv_{self.mod_id_seg}{i}'])
            self.visual_names.extend([f'fake_B_{i}', f'fake_B_{self.mod_id_seg}{i}', f'fake_B_{i}_teacher', f'fake_B_{self.mod_id_seg}{i}_teacher', f'real_B_{i}'])
        self.loss_names.extend([f'G_GAN_{self.mod_id_seg}',f'G_L1_{self.mod_id_seg}',f'D_real_{self.mod_id_seg}',f'D_fake_{self.mod_id_seg}',
                                f'G_KLDiv_{self.mod_id_seg}', f'G_KLDiv_{self.mod_id_seg}{self.opt.modalities_no+1}'])
        self.visual_names.extend([f'fake_B_{self.mod_id_seg}', f'fake_B_{self.mod_id_seg}{self.opt.modalities_no+1}', 
                                f'fake_B_{self.mod_id_seg}_teacher', f'fake_B_{self.mod_id_seg}{self.opt.modalities_no+1}_teacher', f'real_B_{self.mod_id_seg}'])
    ...
    def backward_G(self):
        ...
        for i in range(self.opt.modalities_no):
            setattr(self,f'loss_G_KLDiv_{self.mod_id_seg}{i+1}',self.criterionKLDiv(self.logsoftmax(getattr(self,f'fake_B_{self.mod_id_seg}_{i+1}').view(1,1,-1)), self.softmax(getattr(self,f'fake_B_{self.mod_id_seg}_{i+1}_teacher').view(1,1,-1))))
        ...
        self.loss_G += (getattr(self,f'loss_G_KLDiv_{self.mod_id_seg}') + getattr(self,f'loss_G_KLDiv_{self.mod_id_seg}{self.opt.modalities_no+1}')) * factor_KLDiv
