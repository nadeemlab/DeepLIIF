...
@click.option('--modalities_no', default=4, type=int, help='number of targets')
# model parameters
@click.option('--model', default='DeepLIIF', help='name of model class')
@click.option('--input_nc', default=3, help='# of input image channels: 3 for RGB and 1 for grayscale')
@click.option('--output_nc', default=3, help='# of output image channels: 3 for RGB and 1 for grayscale')
...
@click.option('--net_d', default='n_layers',
...
@click.option('--net_g', default='resnet_9blocks',
              help='specify generator architecture [resnet_9blocks | resnet_6blocks | unet_512 | unet_256 | unet_128]')
@click.option('--n_layers_d', default=4, help='only used if netD==n_layers')
@click.option('--norm', default='batch',
              help='instance normalization or batch normalization [instance | batch | none]')
@click.option('--init_type', default='normal',
              help='network initialization [normal | xavier | kaiming | orthogonal]')
@click.option('--init_gain', default=0.02, help='scaling factor for normal, xavier and orthogonal.')
@click.option('--no_dropout', is_flag=True, help='no dropout for the generator')
# dataset parameters
@click.option('--direction', default='AtoB', help='AtoB or BtoA')
@click.option('--serial_batches', is_flag=True,
              help='if true, takes images in order to make batches, otherwise takes them randomly')
@click.option('--num_threads', default=4, help='# threads for loading data')
@click.option('--batch_size', default=1, help='input batch size')
@click.option('--load_size', default=512, help='scale images to this size')
@click.option('--crop_size', default=512, help='then crop to this size')
@click.option('--max_dataset_size', type=int,
...
@click.option('--no_flip', is_flag=True,
              help='if specified, do not flip the images for data augmentation')
@click.option('--display_winsize', default=512, help='display window size for both visdom and HTML')
# additional parameters
@click.option('--epoch', default='latest',
              help='which epoch to load? set to latest to use latest cached model')
@click.option('--load_iter', default=0,
...
@click.option('--lambda_L1', default=100.0, help='weight for L1 loss')
@click.option('--is_train', is_flag=True, default=True)
@click.option('--continue_train', is_flag=True, help='continue training: load the latest model')
@click.option('--epoch_count', type=int, default=0,
...
@click.option('--n_epochs_decay', type=int, default=100,
...
@click.option('--lr_policy', default='linear',
              help='learning rate policy. [linear | step | plateau | cosine]')
@click.option('--lr_decay_iters', type=int, default=50,
...
@click.option('--display_freq', default=400, help='frequency of showing training results on screen')
@click.option('--display_ncols', default=4,
              help='if positive, display all images in a single visdom web panel with certain number of images per row.')
@click.option('--display_id', default=1, help='window id of the web display')
@click.option('--display_server', default="http://localhost", help='visdom server of the web display')
@click.option('--display_env', default='main',
              help='visdom display environment name (default is "main")')
@click.option('--display_port', default=8097, help='visdom port of the web display')
@click.option('--update_html_freq', default=1000, help='frequency of saving training results to html')
@click.option('--print_freq', default=100, help='frequency of showing training results on console')
@click.option('--no_html', is_flag=True,
              help='do not save intermediate training results to [opt.checkpoints_dir]/[opt.name]/web/')
# network saving and loading parameters
@click.option('--save_latest_freq', default=500, help='frequency of saving the latest results')
@click.option('--save_epoch_freq', default=100,
              help='frequency of saving checkpoints at the end of epochs')
@click.option('--save_by_iter', is_flag=True, help='whether saves model by iteration')
...
@click.option('--remote_transfer_cmd', type=str, default=None, help='module and function to be used to transfer remote files to target storage location, for example mymodule.myfunction')
...
# DeepLIIFExt params
@click.option('--seg_gen', type=bool, default=True, help='True (Translation and Segmentation), False (Only Translation).')
@click.option('--net_ds', type=str, default='n_layers',
...
@click.option('--gan_mode_s', type=str, default='lsgan',
