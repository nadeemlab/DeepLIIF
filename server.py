...
DEFAULT_PORT = 8097
DEFAULT_HOSTNAME = os.environ['HOSTNAME']#"localhost"
DEFAULT_BASE_URL = "/"
...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def render(self, template_name, **kwargs):
        """
        Replaces static_url funciton with CPD specific function
        """
        def cpd_static_url(path):
            return 'static/' + path
        kwargs['static_url'] = cpd_static_url
...
def start_server(port=DEFAULT_PORT, hostname=DEFAULT_HOSTNAME,
    ...
    if print_func is None:
        ...
        # identify if this is a CPD on prem env
        if 'JUPYTER_SERVER_URL' in os.environ:
            if 'dsx' in os.environ['JUPYTER_SERVER_URL']:
                print('You are using IBM Cloud Pak for Data on prem.')
                print('The visdom server is started in Watson Studio JupyterLab environment.')
                try: 
                    from ws_applications import make_link
                    print(f'You can navigate to {make_link(port)}')
                except:
                    print('Make sure you are using a custom image that exposes the app link.')
    else:
        print_func(port)
